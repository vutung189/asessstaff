		...
		<div id="page-wrapper">
			<div class="row">
				<div class="col-sm-12">
						...
						<form:form method="GET" action="search">
							...
							<div class="input-group">
								<input type="text" name="searchString" id="searchString"
									class="form-control" value='${searchString}'
					...
					<c:url value="${contextPath}" var="firstUrl">
					  <c:param name="searchString"   value="${searchString}" />
					  <c:param name="pageNumber"    value="1" />
					</c:url>
					
					<c:url value="${contextPath}" var="lastUrl">
					  <c:param name="searchString"   value="${searchString}" />
					  <c:param name="pageNumber"    value="${page.totalPages}" />
					</c:url>	
					
					<c:url value="${contextPath}" var="prevUrl">
					  <c:param name="searchString"   value="${searchString}" />
					  <c:param name="pageNumber"    value="${currentIndex - 1}" />
					</c:url>						
						
					<c:url value="${contextPath}" var="nextUrl">
					  <c:param name="searchString"   value="${searchString}" />
					  <c:param name="pageNumber"    value="${currentIndex + 1}" />
					</c:url>						
					...
					<ul class="pagination">
						...
						<c:forEach var="i" begin="${beginIndex}" end="${endIndex}">
							...
							<c:url value="${contextPath}" var="pageUrl">
							  <c:param name="searchString"   value="${searchString}" />
